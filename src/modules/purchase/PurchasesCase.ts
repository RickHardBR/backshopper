...
export class PurchasesCase {
  constructor(private purchaseModel: IPurchaseModel) {}

  async purchase(email: string, request: RequestType) {
    let { delivey_date, listPurchases } = request
    ...
    const soldOutProducts = await this.purchaseModel.soldOutProducts(
      listPurchases
    )

    if (soldOutProducts.length > 0) {
      throw new ErrorQtyStock(
